...
/// <summary>
///     Represents an opaque continuation token for paginating the authenticated user's watch history feed.
/// </summary>
public sealed class HistoryContinuation
{
    private const string RouteName = "history";

    internal HistoryContinuation(string token, string? profileId = null, string? trackingParams = null)
    {
    ...
    /// <summary>
    ///     Imports and validates a previously exported history continuation string.
    /// </summary>
    /// <param name="value">The exported continuation string.</param>
    /// <returns>A validated <see cref="HistoryContinuation" /> instance.</returns>
    ...
    public static HistoryContinuation Import(string value)
    {
        ...
        return !string.Equals(envelope.Route, RouteName, StringComparison.Ordinal)
            ...
            : new HistoryContinuation(envelope.Token, envelope.ProfileId, envelope.TrackingParams);
    }
}